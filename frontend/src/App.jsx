...
export default function App() {
  ...
  const [formData, setFormData] = useState({ url: '', goal: '', context: '' });
                      ...
                      <div style={{ maxHeight: '600px', overflowY: 'auto', paddingRight: '8px' }}>
                         {(selectedTest?.screenshots || []).map((s, i) => (
                           <div key={i} className="screenshot-card" style={{ marginBottom: '16px' }}>
                              <img src={(s && typeof s === 'string' && s.startsWith('data:')) ? s : `data:image/png;base64,${s}`} style={{ width: '100%', borderRadius: '8px' }} />
